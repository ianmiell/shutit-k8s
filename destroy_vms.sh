...
then
	while true
	do
		VBoxManage list runningvms | grep ${MODULE_NAME} | awk '{print $1}' | xargs $XARGS_FLAG -IXXX VBoxManage controlvm 'XXX' poweroff && VBoxManage list vms | grep shutit_k8s | awk '{print $1}'  | xargs -IXXX VBoxManage unregistervm 'XXX' --delete
